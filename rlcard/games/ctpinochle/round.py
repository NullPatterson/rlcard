...
class CTPinochleRound:    
    ...
    def is_bidding_over(self) -> bool:
        ...
        return self.player_pass.count(True) == 2 or self.bid_round > 2 # Using 0 index as first index for bid_round count
    ...
    def make_call(self, action: CallActionEvent): # FLAG I need to figure out where players get to pass and make sure if they have passed already
        ...
        if isinstance(action, PassBid):
            if self.player_pass[self.current_player_id]:
            ...
            self.pass_count += 1
            self.player_pass[self.current_player_id] = True
        ...
        if not self.is_bidding_over():
            ...
            while self.player_pass[next_player]:
    ...
    def print_scene(self):
        print(f'=====Dealer {self.dealer_id} | Move: {len(self.move_sheet)} | Player: {self.players[self.current_player_id]} | Phase: {self.roundphase} =====')
        ...
        if self.is_bidding_over() and self.winning_bid_move:
            ...
            if not trump_suit:
                trump_suit = 'NT'
            print(f'Bid Winner: {self.bid_winner_id} | Bid Amount: {self.winning_bid_move.action.bid_amount} | Trump Suit: {trump_suit}')

        for player in self.players:
            print(f'{player}: {[str(card) for card in player.hand]}')
        if self.is_bidding_over():
            print(self.get_trick_moves())